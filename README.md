...
For a hosted single-instance deployment, [`render.yaml`](render.yaml) provisions the Docker service in Singapore with a persistent `/app/.data` disk. Render asks for the Owner email and Owner password during the initial Blueprint setup; the session signing secret is generated by the platform and the public HTTPS origin is detected from Render at runtime. Persistent disks require a paid service plan.
